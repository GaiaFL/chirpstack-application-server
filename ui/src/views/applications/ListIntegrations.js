...
import KonkerCard from "./integrations/KonkerCard";
...
class ListIntegrations extends Component {
  ...
  loadIntegrations = (organizationID, applicationID) => {
    ApplicationStore.listIntegrations(applicationID, (resp) => {
      ...
      // Konker
      if(includes(resp.result, "KONKER")) {
        configured.push(<KonkerCard organizationID={organizationID} applicationID={applicationID} />);
      } else {
        available.push(<KonkerCard organizationID={organizationID} applicationID={applicationID} add />);
      }
